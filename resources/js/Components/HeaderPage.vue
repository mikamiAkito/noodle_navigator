...
<template>
  <div>
    <div class="bg-orange-600 absolute z-10 w-full">
      aaaaaaaaaaa
    </div>
    ...
    <div id="g-nav" v-show="Hambarg" :class="['fixed', 'z-30', 'right-12', 'top-12',{active:Hambarg}]">
      <ul class="opacity-0" :class="['opacity-100', {active:Hambarg}]">
        <li class="cursor-pointer text-white hover:text-slate-300 duration-500 pb-2 font-black text-lg"
        :class="['animate-gnaviAnime', 'duration-1000', 'delay-200', '']">
        <a :href="a">プロフィール</a>
        </li>
        <li class="cursor-pointer text-white hover:text-slate-300 duration-500 pb-2 font-black text-lg"
        :class="['animate-gnaviAnime', 'duration-1000', 'delay-200', '']">
    ...
    <div class="circle-bg scale-150 bg-orange-400 fixed size-20 sm:size-24 rounded-full z-20 -right-12 -top-12 duration-700"
    :class="{'scale-450':Hambarg}">
    </div>
    <div id="z-black" @click="Hambarg = !Hambarg" class="fixed w-full invisible duration-700" :class="{'bgaction':Hambarg}"></div>
  </div>
</template>
